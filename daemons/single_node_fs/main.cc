...
void chfs_read(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off, struct fuse_file_info *fi) {
  ...
  fuse_reply_buf(req, reinterpret_cast<const char *>(res_data.data()), read_size);
}
  ...
  if (res.is_err()) {
    ...
    switch (error_code) {
      case ErrorType::OUT_OF_RESOURCE:
        std::cerr << "OUT_OF_RESOURCE, free blocks left: " << fs->get_free_blocks_num().unwrap() << std::endl;
        break;
      default:
        UNIMPLEMENTED();
        break;
...
auto bootstrap_fuse(int argc, char **argv) -> int {
  ...
  auto bm = std::shared_ptr<BlockManager>(new BlockManager(kDiskSize / KBlockSize, KBlockSize));
