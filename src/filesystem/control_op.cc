...
FileOperation::FileOperation(std::shared_ptr<BlockManager> bm, u64 max_inode_supported)
    : block_manager_(bm),
      inode_manager_(std::shared_ptr<InodeManager>(new InodeManager(bm, max_inode_supported))),
      block_allocator_(std::shared_ptr<BlockAllocator>(new BlockAllocator(bm, inode_manager_->get_reserved_blocks()))) {
  // now initialize the superblock
  SuperBlock(bm, inode_manager_->get_max_inode_supported()).flush(0).unwrap();
}

auto FileOperation::create_from_raw(std::shared_ptr<BlockManager> bm) -> ChfsResult<std::shared_ptr<FileOperation>> {
  // 1. get the metadata from the super block
  auto superblock_res = SuperBlock::create_from_existing(bm, 0);
  if (superblock_res.is_err()) {
    return ChfsResult<std::shared_ptr<FileOperation>>(superblock_res.unwrap_error());
  }

  // 2. create the innode manager
  auto inode_manager_res = InodeManager::create_from_block_manager(bm, superblock_res.unwrap()->get_ninodes());
  if (inode_manager_res.is_err()) {
    return ChfsResult<std::shared_ptr<FileOperation>>(inode_manager_res.unwrap_error());
  }

  auto reserved_block_num = inode_manager_res.unwrap().get_reserved_blocks();
  return ChfsResult<std::shared_ptr<FileOperation>>(std::shared_ptr<FileOperation>(
      new FileOperation(bm, InodeManager::to_shared_ptr(inode_manager_res.unwrap()),
                        std::shared_ptr<BlockAllocator>(new BlockAllocator(bm, reserved_block_num, false)))));
}

auto FileOperation::get_free_inode_num() const -> ChfsResult<u64> { return inode_manager_->free_inode_cnt(); }
...
auto FileOperation::remove_file(inode_id_t id) -> ChfsNullResult {
  ...
  if (inode_p->blocks[inode_p->get_direct_block_num()] != KInvalidBlockID) {
    ...
    auto read_res =
        this->block_manager_->read_block(inode_p->blocks[inode_p->get_direct_block_num()], indirect_block.data());
    if (read_res.is_err()) {
