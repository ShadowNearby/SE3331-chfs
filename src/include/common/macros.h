...
#define CHFS_VERIFY(expr, message)                                                                             \
  do {                                                                                                         \
    if (!(expr)) {                                                                                             \
      std::cerr << "Chfs fatal invariant check failed: " << (message) << " at " << __FILE__ << ":" << __LINE__ \
                << std::endl;                                                                                  \
      std::abort();                                                                                            \
    }                                                                                                          \
  } while (0)
...
#define UNIMPLEMENTED()                                                                                             \
  do {                                                                                                              \
    std::cerr << "Unimplemented function of chfs: " << __PRETTY_FUNCTION__ << " at " << __FILE__ << ":" << __LINE__ \
              << std::endl;                                                                                         \
    std::abort();                                                                                                   \
  } while (0)
