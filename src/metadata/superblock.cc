...
SuperBlock::SuperBlock(std::shared_ptr<BlockManager> bm, u64 ninodes) : bm(bm) {
  ...
  CHFS_VERIFY(this->inner.block_size >= sizeof(SuperBlockInternal), "Block size too small");
}

auto SuperBlock::create_from_existing(std::shared_ptr<BlockManager> bm, block_id_t id)
