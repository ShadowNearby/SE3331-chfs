...
TEST_F(InodeTest, Init) {
  ...
  auto file_sz_supported_by_one_block =
      static_cast<u64>(TEST_BLOCK_SZ / sizeof(block_id_t)) * static_cast<u64>(TEST_BLOCK_SZ);
  auto file_in_inode =
      static_cast<u64>((TEST_BLOCK_SZ - sizeof(Inode)) / sizeof(block_id_t) - 1) *  // 1: the indirect block
      static_cast<u64>(TEST_BLOCK_SZ);
  ASSERT_EQ(inode.max_file_sz_supported(), file_sz_supported_by_one_block + file_in_inode);
}
